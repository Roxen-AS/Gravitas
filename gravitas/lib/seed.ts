...
const db = drizzle({ client: rawSql, schema })

async function seed() {
  console.log('🌱 Creating tables...')

  await rawSql(`CREATE TABLE IF NOT EXISTS components (
    id TEXT PRIMARY KEY,
    label TEXT NOT NULL,
  ...
  await rawSql(`CREATE TABLE IF NOT EXISTS edges (
  ...
  await rawSql(`CREATE TABLE IF NOT EXISTS events (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  await rawSql(`CREATE TABLE IF NOT EXISTS governance (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  const decisions = [
    ...
    },
    {
      ...
      assumptions: ['< 10K writes/hr for 18 months', 'ACID guarantees required', 'No unstructured storage needed'],
    ...
    },
    {
      ...
      rationale: 'Reused existing Redis infra to avoid additional managed service, accepting reduced durability vs SQS',
  ...
  for (const d of decisions) {
    ...
    console.log(`  ✓ ${d.component}: "${d.choice}"`)
  }

  console.log('\n✅ Seed complete — 5 components, 5 edges, 5 decisions.')
  process.exit(0)
}
